...
uint8_t init_heap[PAGE_SIZE * 3] __attribute__((aligned(PAGE_SIZE)));
...
uint64_t addr_virt_to_phys(uint64_t virt_addr)
{
    ...
    return (virt_addr & 0xFFF) + (pt[pt_idx] & ~0xFFF);
}
void init_map_page(uint64_t phys_addr, uint64_t virt_addr)
{
    phys_addr &= ~0xFFF;
    virt_addr &= ~0xFFF;
    uint16_t pml4_idx = (virt_addr >> 39) & 0x1FF;
    ...
    {
        uint64_t *pdpt = (void *)&init_heap[0];
    ...
    {
        uint64_t *pd = (void *)&init_heap[PAGE_SIZE];
    ...
    {
        uint64_t *pt = (void *)&init_heap[PAGE_SIZE * 2];
